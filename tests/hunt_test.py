...
class HuntResultsTest(BaseTestCase):
    ...
    def test_live_hunt_results_json(self):
        with patch('polyswarm_api.api.PolyswarmAPI.live_results', return_value=[resources.live_results(self)[0]]):
            result = self._run_cli(['--output-format', 'json',
                                    'live', 'results', '63433636835291189',
                                    '--since', '9999999'])
        self._assert_json_result(
            result,
            expected_output=resources.live_results(self)[0].json,
            expected_return_code=0,
        )

    def test_historical_hunt_results_json(self):
        with patch('polyswarm_api.api.PolyswarmAPI.historical_results', return_value=[resources.hisotrical_results(self)[0]]):
            result = self._run_cli(['--output-format', 'json',
                                    'historical', 'results', '47190397989086018'])
        self._assert_json_result(
            result,
            expected_output=resources.hisotrical_results(self)[0].json,
            expected_return_code=0,
        )
...
class LiveHuntTest(BaseTestCase):
    ...
    def test_live_hunt_create_json(self):
        with patch('polyswarm_api.api.PolyswarmAPI.live_create', return_value=resources.hunts(self)[0]):
            result = self._run_cli(['--output-format', 'json',
                                    'live', 'start', os.path.join(BASE_PATH, 'eicar.yara')])
        self._assert_json_result(
            result,
            expected_output=resources.hunts(self)[0].json,
            expected_return_code=0,
        )
    ...
    def test_live_hunt_create_text(self):
        ...
        self._assert_text_result(
            result,
            expected_output=resources.text_hunts()[0],
            expected_return_code=0,
        )

    def test_live_hunt_delete_json(self):
        with patch('polyswarm_api.api.PolyswarmAPI.live_delete', return_value=resources.hunts(self)[0]):
            result = self._run_cli(['--output-format', 'json',
    ...
    def test_live_hunt_delete_text(self):
        ...
        self._assert_text_result(
            result,
            expected_output=resources.text_detele_hunts()[0],
            expected_return_code=0,
        )

    def test_live_hunt_list_json(self):
        with patch('polyswarm_api.api.PolyswarmAPI.live_list', return_value=[resources.hunts(self)[0]]):
            result = self._run_cli(['--output-format', 'json',
                                    'live', 'list'])
        self._assert_json_result(
            result,
...
class HistoricalHuntTest(BaseTestCase):
    ...
    def test_historical_hunt_create_json(self):
        with patch('polyswarm_api.api.PolyswarmAPI.historical_create', return_value=resources.hunts(self)[0]):
            result = self._run_cli(['--output-format', 'json',
                                    'historical', 'start', os.path.join(BASE_PATH, 'eicar.yara')])
        self._assert_json_result(
            result,
            expected_output=resources.hunts(self)[0].json,
            expected_return_code=0,
    ...
    def test_historical_hunt_delete_json(self):
        with patch('polyswarm_api.api.PolyswarmAPI.historical_delete', return_value=resources.hunts(self)[0]):
            result = self._run_cli(['--output-format', 'json',
    ...
    def test_historical_hunt_list_json(self):
        with patch('polyswarm_api.api.PolyswarmAPI.historical_list', return_value=[resources.hunts(self)[0]]):
            result = self._run_cli(['--output-format', 'json',
                                    'historical', 'list'])
        self._assert_json_result(
            result,
