...
class SubmissionTest(BaseTestCase):
    ...
    def test_submission_lookup_json(self):
        with patch('polyswarm_api.api.PolyswarmAPI.submit', return_value=[
            mock_polyswarm_api_results.submissions(self)[0]]), \
             patch('polyswarm_api.api.PolyswarmAPI.lookup',
                   return_value=iter([mock_polyswarm_api_results.submissions(self)[0]])):
    ...
    def test_submission_lookup_text(self):
        with patch('polyswarm_api.api.PolyswarmAPI.lookup',
                   return_value=iter([mock_polyswarm_api_results.submissions(self)[0]])), \
    ...
    def test_submission_create_json(self):
        with patch('polyswarm_api.api.PolyswarmAPI.submit',
                   return_value=[mock_polyswarm_api_results.submissions(self)[0]]), \
             patch('polyswarm_api.api.PolyswarmAPI.lookup',
                   return_value=iter([mock_polyswarm_api_results.submissions(self)[0]])):
            result = self._run_cli(['--output-format', 'json', '-c', 'gamma',
